-- 0033_jazyk_label_views.sql
-- Part B/C — render the JAZYK code (Podnet nad rámec § 44 — jazyk) in the public
-- views. JAZYK is NOT a § 44 condition: it must show with its own label, sort
-- AFTER P-f (order 10), and is excluded from the semafor (it never appears in
-- district_compositions / composition_color). Recreate scorecard + findings views
-- with the JAZYK label CASE added. Idempotent (CREATE OR REPLACE / DROP+CREATE).

-- ---------------------------------------------------------------------------
-- district_scorecard — add JAZYK label + order
-- ---------------------------------------------------------------------------
 ...
 SELECT lv.district_id,
    ...
    lv.condition_code,
        CASE lv.condition_code
            ...
            WHEN 'JAZYK'::text THEN 'Podnet nad rámec § 44 (jazyk)'::text
